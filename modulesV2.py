...
def normal(prcp_d, startDay, endDay, wtr_mng_d, pet_d, ditch_par, pond_par, F_doy_array, fert_type_array,
    ...
    if orderDitch == 1:
        h_yan = int(0.5 * HDiff_FD1)
    else:
        h_yan = int(0.5 * HDiff_FD2 + HDiff_FD1 - HDiff_FD2)
    ...
    for column in prcp_d.columns:
        ...
        while date_n < len(prcp_d.index):
            ...
            if (dayofYear < startday_doy) | (dayofYear > endday_doy):
                ...
                Q_dp = sd_functions.f_vhpd(Hdp, volume_add, h_v_sr, area_pd, h_yan)[0]  # in m3/d
                Hdp1 = sd_functions.f_vhpd(Hdp, volume_add, h_v_sr, area_pd, h_yan)[1]  # in mm
...
def normal_irrigation_restrict(prcp_d, startDay, endDay, wtr_mng_d, pet_d, ditch_par, pond_par, F_doy_array, fert_type_array,
           TN0_array, TP0_array, TN_I, TP_I, area_f, Ks, Fc0_f, Fc0_pd, TN_P0, TN_P1, TP_P, TN_rate_lch, TN_rate_lch_lat,
           TP_rate_lch, TP_rate_lch_lat, ENC0_dp, EPC0_dp, ENCR_f, EPCR_f, vf_N_array, vf_P, h_v_sr, vf_N_pd, vf_P_pd, run_id,
           TAWC, Irrg_rate, output_dir=0, graph=0):
    """ The normal mode, for only accurate water level management.
    Parameters:
        prcp_d, pandas DataFrame stores daily precipitation data, in mm;
    Outputs:
    ...
    # The rate of area of ditches and pond to the total area, in m2
    area_pd = area_d1 + area_d2 + area_p
    Yield_df = pd.DataFrame(index=range(1988, 2018, 1), columns=prcp_d.columns, dtype=np.float32)
    startday_doy = pd.Timestamp(2018, int(startDay.split('-')[0]), int(startDay.split('-')[1])).dayofyear
    endday_doy = pd.Timestamp(2018, int(endDay.split('-')[0]), int(endDay.split('-')[1])).dayofyear
    for column in prcp_d.columns:
        date_n = 0
        while date_n < len(prcp_d.index):
            dayofYear = prcp_d.index[date_n].dayofyear
            year = prcp_d.index[date_n].year
            monthDay = str(prcp_d.index[date_n].month) + "-" + str(prcp_d.index[date_n].day)
            irrg_date = wtr_mng_d["H_min"].ne(0).idxmax()
            irrg_doy = pd.Timestamp(2018, int(irrg_date.split('-')[0]), int(irrg_date.split('-')[1])).dayofyear
            if dayofYear == irrg_doy:
                ponding_date = date_n
            if dayofYear == 1:
                ET_yield_df = pd.DataFrame(index=wtr_mng_d.index, columns=["ET", "ETc", "Ky"], dtype=np.float)
                ET_yield_df.loc[:, "Ky"] = wtr_mng_d["Ky"]
            if calendar.isleap(prcp_d.index[date_n].year):
                dayofYear = dayofYear - 1
            if dayofYear == 365:
                """Rice yield reduction rate"""
                ET_yield = ET_yield_df.groupby("Ky").sum()
                ET_yield["yield_red"] = (1 - ET_yield["ET"] / ET_yield["ETc"]) * ET_yield.index
                Yield_df.loc[year, column] = ET_yield["yield_red"].sum() / (ET_yield.index.values.sum())
            if dayofYear == startday_doy:
                Hf = 0.0
                Hdp = Hd10
                Hd1 = Hd10
                TP_cd1 = TP_cd10
                TN_cd1 = TN_cd10
                Dr = 0.0  # root zone depletion, in mm
                if orderDitch == 2:
                    Hd2 = Hd1 + HDiff_FD2 - HDiff_FD1
                    TP_cd2 = TP_cd20
                    TN_cd2 = TN_cd20
            ...
            if (dayofYear < startday_doy) | (dayofYear > endday_doy):
                ...
                PET = pet_d[column].iloc[date_n]
                # Calculate ET
                ETc = PET * wtr_mng_d.loc[monthDay, 'Kc']
                p = max(0.1, 0.2 + 0.04 * (5 - ETc))
                p = min(0.8, p)
                TAW = TAWC * wtr_mng_d.loc[monthDay, "Zr"] * 1000  # total available water, in mm
                RAW = p * TAW  # readily available water, in mm
                if Hf > 0:
                    ET = ETc
                else:
                    if Dr <= RAW:
                        ...
                        K_ets = (TAW - Dr) / (TAW - RAW)
                    ET = ETc * K_ets
                ET_df[column].iloc[date_n] = ET
                ET_yield_df.loc[monthDay, "ET"] = ET
                ET_yield_df.loc[monthDay, "ETc"] = ETc
                # Do irrigation or not
                if (wtr_mng_d.loc[monthDay, 'H_min'] != 0) & (Hf < wtr_mng_d.loc[monthDay, 'H_min']) & (
                        prcp + Hf - ET < wtr_mng_d.loc[monthDay, 'H_min']):
                    irrg = wtr_mng_d.loc[monthDay, 'H_max'] - Hf
                    ...
                    irrg = 0
                I_need_df[column].iloc[date_n] = irrg
                # Adjust irrigation quantity and frequency in 25% dry conditions
                if (irrg > 0) & (dayofYear != irrg_doy):
                    prcp_cum = prcp_d1[column].iloc[date_n-60: date_n].sum()
                    I_df1 = I_df[column].copy(deep=True)
                    I_df1.iloc[ponding_date] = 0
                    Irrg_cum = I_df1.iloc[date_n-60: date_n].sum()
                    if Irrg_cum > prcp_cum * Irrg_rate[0]:
                        if prcp_cum == 0:
                            irrg = 0
                        else:
                            irrg_adj = max(0, (Irrg_rate[1] - Irrg_cum / prcp_cum) / (Irrg_rate[1] - Irrg_rate[0]))
                            irrg = irrg_adj * irrg
                    elif Irrg_cum + irrg > prcp_cum * Irrg_rate[1]:
                        irrg = prcp_cum * Irrg_rate[1] - Irrg_cum
                I_df[column].iloc[date_n] = irrg
                # Calculate lateral seepage to ditches
                if Hf > 0:
                    if orderDitch == 2:
                    ...
                    F_lat_d1 = 0.0
                    F_lat = 0.0  # When the root zone is not saturated, no lateral flow
                    if orderDitch == 2:
                        F_lat_d2 = 0.0
                ...
                if havePond == True:
                    if Hp > 0:
                        ...
                        if Hp > -9:
                            ...
                            Kr_p = max(0, (20 + Hp)/(20.0 - 9.0))
                # ET, ET_pd;, downward percoloation, Fc_pd
                if (havePond is True) & (orderDitch == 2):
                    if (Hd1 <= 0) & (Hd2 <= 0) & (Hp <= 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + Kr_d2 * PET * area_d2 + Kr_p * PET * area_p
                    elif (Hd1 <= 0) & (Hd2 <= 0) & (Hp > 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + Kr_d2 * PET * area_d2 + kc_p * PET * area_p_d + \
                                  0.60 * PET * (area_p - area_p_d)
                    elif (Hd1 <= 0) & (Hd2 > 0) & (Hp > 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + kc_d2 * PET * area_d2_d + kc_p * PET * area_p_d + \
                                  0.60 * PET * (area_d2 - area_d2_d + area_p - area_p_d)
                    else:
                        ET_pd_v = kc_d1 * PET * area_d1_d + kc_d2 * PET * area_d2_d + kc_p * PET * area_p_d + \
                                  0.60 * PET * (area_pd - area_d1_d - area_d2_d - area_p_d)
                    Fc_pd_v = Fc_d1_v + Fc_d2_v + Fc_p_v
                    ...
                    if (Hd1 <= 0) & (Hp <= 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + Kr_p * PET * area_p
                    elif (Hd1 <= 0) & (Hp > 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + kc_p * PET * area_p_d + \
                                  0.60 * PET * (area_p - area_p_d)
                    else:
                        ET_pd_v = kc_d1 * PET * area_d1_d + kc_p * PET * area_p_d + 0.60 * PET * (
                                area_pd - area_d1_d - area_p_d)
                    ...
                    if (Hd1 <= 0) & (Hd2 <= 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + Kr_d2 * PET * area_d2
                    elif (Hd1 <= 0) & (Hd2 > 0):
                        ET_pd_v = Kr_d1 * PET * area_d1 + kc_d2 * PET * area_d2_d + \
                                  0.60 * PET * (area_d2 - area_d2_d)
                    else:
                        ET_pd_v = kc_d1 * PET * area_d1_d + kc_d2 * PET * area_d2_d + 0.60 * PET * (
                                area_pd - area_d1_d - area_d2_d)
                    Fc_pd_v = Fc_d1_v + Fc_d2_v
                    ...
                    if Hd1 <= 0:
                        ET_pd_v = Kr_d1 * PET * area_d1
                    else:
                        ET_pd_v = kc_d1 * PET * area_d1_d + 0.60 * PET * (area_pd - area_d1_d)
                    Fc_pd_v = Fc_d1_v
                ...
                if prcp_cum == 0:
                    ...
                    h_yan_adj = (Irrg_rate[1] - Irrg_cum / prcp_cum) / (Irrg_rate[1] - Irrg_rate[0]) * 0.4 + 0.1
                    h_yan_adj = max(h_yan_adj, 0)
                    h_yan_adj = min(h_yan_adj, 0.8)
                if orderDitch == 1:
                    h_yan = int(h_yan_adj * HDiff_FD1)
                else:
                    h_yan = int(h_yan_adj * HDiff_FD2 + HDiff_FD1 - HDiff_FD2)
                Q_dp = sd_functions.f_vhpd(Hdp, volume_add, h_v_sr, area_pd, h_yan)[0]  # in m3/d
                Hdp1 = sd_functions.f_vhpd(Hdp, volume_add, h_v_sr, area_pd, h_yan)[1]  # in mm
                Qdp_df[column].iloc[date_n] = Q_dp / (area_f + area_pd) * 1000  # in mm/d
                Hdp_df[column].iloc[date_n] = Hdp1
                ...
                # Calculate mixed concentration
                if orderDitch == 2:
                    v_temp = D * Wf1 * Wf2 + F_lat_d1 + sd_functions.f_vhd(
                        Hd1, HDiff_FD1, Wd1_up, Wd1_lw, Wf2) + (prcp * area_d1 - kc_d1 * PET * area_d1_d) / n1 / n2
                    if v_temp > 0:
                        TN_cd1_out = (TN_cf * D * Wf1 * Wf2 + TN_cfb_lat * F_lat_d1 + TN_cd1 * sd_functions.f_vhd(
                            Hd1, HDiff_FD1, Wd1_up, Wd1_lw, Wf2) + TN_P * prcp * area_d1 / n1 / n2
                ...
                if orderDitch == 2:
                    ...
                    if D > 0:
                        ...
                        Q_d1_0 = (D * Wf1 * Wf2 + F_lat_d1 + sd_functions.f_vhd(Hd1, HDiff_FD1, Wd1_up, Wd1_lw, Wf2)
                                  + (prcp * area_d1 - kc_d1 * PET * area_d1_d - Fc_d1_v) / n1 / n2 - sd_functions.f_vhd(
                                    Hdp1, HDiff_FD1, Wd1_up, Wd1_lw, Wf2)) / 1000  # in m3
                        v_temp = F_lat_d2 + sd_functions.f_vhd(
                            Hd2, HDiff_FD2, Wd2_up, Wd2_lw, Wf1) + (prcp * area_d2 - kc_d2 * PET * area_d2_d
                                                                    ) / n1 / n2 + Q_d1_0 * 1000
                        if v_temp > 0:
                            TN_cd2_mix = (TN_cfb_lat * F_lat_d2 + TN_cd2 * sd_functions.f_vhd(
                        ...
                        # Calculate mixed concentration with lateral flow, precipitation and ET
                        v_temp = F_lat_d2 + sd_functions.f_vhd(Hd2, HDiff_FD2, Wd2_up, Wd2_lw, Wf1) + (
                                prcp * area_d2 - kc_d2 * PET * area_d2_d) / n1 / n2
                        if v_temp > 0:
                            TN_cd2_mix = (TN_cfb_lat * F_lat_d2 + TN_cd2 * sd_functions.f_vhd(
                                Hd2, HDiff_FD2, Wd2_up, Wd2_lw, Wf1) + TN_P * prcp * area_d2 / n1 / n2) / v_temp
                            TP_cd2_mix = (TP_cfb_lat * F_lat_d2 + TP_cd2 * sd_functions.f_vhd(
                                Hd2, HDiff_FD2, Wd2_up, Wd2_lw, Wf1) + TP_P * prcp * area_d2 / n1 / n2) / v_temp
                        else:
                            TN_cd2_mix = TN_cd2
                            TP_cd2_mix = TP_cd2
                        if TN_cd2_mix <= 0:
                ...
                if havePond == True:
                    ...
                    if orderDitch == 2:
                        ...
                        # mix flow from ditches with the water in ponds and precipitation water, in mg/L
                        if total_volume_d > total_volume_d_bln:
                            v_temp = total_volume_d - total_volume_d_bln + sd_functions.f_vhp(
                                Hp, HDiff_FP, area_p, slp_p) + prcp * area_p - kc_p * PET * area_p_d
                            if v_temp > 0:
                                TN_cp_mix = (TN_cd2_out * (total_volume_d - total_volume_d_bln) + TN_cp *
                                             sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) +
                                             TN_P * prcp * area_p) / v_temp
                                TP_cp_mix = (TP_cd2_out * (total_volume_d - total_volume_d_bln) + TP_cp *
                                             sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) +
                                             TP_P * prcp * area_p) / v_temp
                            else:
                                TN_cp_mix = TN_cp
                                TP_cp_mix = TP_cp
                        else:
                            v_temp = sd_functions.f_vhp(
                                Hp, HDiff_FP, area_p, slp_p) + prcp * area_p - kc_p * PET * area_p_d
                            if v_temp > 0:
                                TN_cp_mix = (TN_cp * sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) +
                                             TN_P * prcp * area_p) / v_temp
                        ...
                        if total_volume_d > total_volume_d_bln:
                            ...
                            v_temp = sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) + \
                                     prcp * area_p - kc_p * PET * area_p_d
                            if v_temp > 0:
                                TN_cp_mix = (TN_cp * sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) +
                                             TN_P * prcp * area_p) / v_temp
    ...
    """ Calculate daily concentrations in surface flow from fields and from the IDU outlet """
    TN_cf_out = TN_cf_df.copy(deep=True)
    TN_cf_out[D_df == 0] = np.NAN
    TP_cf_out = TP_cf_df.copy(deep=True)
    TP_cf_out[D_df == 0] = np.NAN
    TN_out_df[Qdp_df == 0] = np.NAN
    TP_out_df[Qdp_df == 0] = np.NAN
    """ Calculate the total TN and TP loads, in kg/ha/d """
    ...
    """ Calculate the total nutrient inputs to and outputs from the field-ditch-pond system unit.
        The inputs are from irrigation and precipitation, the outputs are as surface runoff and leaching."""
    irrg_A = I_df.resample('A', kind='period').sum()
    irrg_need_A = I_need_df.resample('A', kind='period').sum()
    prcp_A = prcp_d.resample('A', kind='period').sum()
    # field scale
    irrg_TN_input_f = irrg_A * TN_I / 100  # in kg/ha/yr
    irrg_TP_input_f = irrg_A * TP_I / 100  # in kg/ha/yr
    ...
    """ Write the results into text files """
    if output_dir != 0:
        # Write yield reduction file
        with open(output_dir + '/yield_reduction.txt', mode='w') as output_f:
            output_f.write(
                'The simulated rice yield reduction rate (unitless) are below:\n')
            output_f.close()
        Yield_df.to_csv(output_dir + '/yield_reduction.txt', mode='a', header=True, index=True, sep='\t',
                          float_format='%.4f')
        # Write runoff and leaching loads for water footprint calculation
        with open(output_dir + '/loads_4gwf{}.txt'.format(run_id), mode='w') as output_f:
            output_f.write(
        ...
        with open(output_dir + '/I_out{}.txt'.format(run_id), 'w') as f:
            ...
            f.close()
        irrg_A.to_csv(output_dir + '/I_out{}.txt'.format(run_id), mode='a', header=True, index=True, sep='\t', float_format='%.2f')
        with open(output_dir + '/I_df.txt', 'w') as f:
            f.write("The irrigated water content is simulated as below, in mm.\n")
            f.close()
        I_df.to_csv(output_dir + '/I_df.txt', mode='a', header=True, index=True, sep='\t', float_format='%.2f')
        with open(output_dir + '/I_need_A{}.txt'.format(run_id), 'w') as f:
            f.write("The irrigated water content needed for rice growing is simulated as below, in mm.\n")
            f.close()
        irrg_need_A.to_csv(output_dir + '/I_need_A{}.txt'.format(run_id), mode='a', header=True, index=True, sep='\t', float_format='%.2f')
        with open(output_dir + '/I_need_df.txt', 'w') as f:
            f.write("The irrigated water content needed for rice growing is simulated as below, in mm.\n")
            f.close()
        I_need_df.to_csv(output_dir + '/I_need_df.txt', mode='a', header=True, index=True, sep='\t', float_format='%.2f')
        # precipitation water content
        with open(output_dir + '/prcp_A.txt', 'w') as f:
            f.write("The precipitation water content during the growing season is simulated as below, in mm.\n")
            f.close()
...
def recycle_irrg_irrigation_restrict(prcp_d, startDay, endDay, wtr_mng_d, pet_d, ditch_par, pond_par, F_doy_array, fert_type_array,
             TN0_array, TP0_array, TN_I, TP_I, area_f, Ks, Fc0_f, Fc0_pd, TN_P0, TN_P1, TP_P, TN_rate_lch, TN_rate_lch_lat,
             TP_rate_lch, TP_rate_lch_lat, ENC0_dp, EPC0_dp, ENCR_f, EPCR_f, vf_N_array, vf_P, h_v_sr, vf_N_pd, vf_P_pd,
             min_level, run_id, TAWC, Irrg_rate, output_dir=0, graph=0):
    """ The recycling irrigation mode.
        Parameters:
            prcp_d, pandas DataFrame stores daily precipitation data, in mm;
        Outputs:
    ...
    I_pond_df = pd.DataFrame(index=prcp_d.index, columns=prcp_d.columns,
                         dtype=np.float32)  # The amount of irrigated water from ponds, in mm/d
    I_out_df = pd.DataFrame(index=prcp_d.index, columns=prcp_d.columns,
                         dtype=np.float32)  # The amount of irrigated water from ponds, in mm/d
    TN_I_out_df = pd.DataFrame(index=prcp_d.index, columns=prcp_d.columns,
                               dtype=np.float32)  # TN load from irrigated water from outside of the system, in mg/m2/d
    TP_I_out_df = pd.DataFrame(index=prcp_d.index, columns=prcp_d.columns,
    ...
    for column in prcp_d.columns:
        ...
        while date_n < len(prcp_d.index):
            ...
            if (dayofYear < startday_doy) | (dayofYear > endday_doy):
                ...
                if (wtr_mng_d.loc[monthDay, 'H_min'] != 0) & (Hf < wtr_mng_d.loc[monthDay, 'H_min']) & (
                    ...
                    if dayofYear == irrg_doy:
                        ...
                        if (havePond is True) & (orderDitch == 2):
                            irrg_pond_max = sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) - sd_functions.f_vhp(
                                min_level, HDiff_FP, area_p, slp_p) + (
                                                        sd_functions.f_vhd(Hd2, HDiff_FD2, Wd2_up, Wd2_lw,
                                                                           Wf1) - sd_functions.f_vhd(
                                                    min_level, HDiff_FD2, Wd2_up, Wd2_lw, Wf1)) * n1 * n2
                        elif (havePond is True) & (orderDitch == 1):
                            irrg_pond_max = sd_functions.f_vhp(Hp, HDiff_FP, area_p, slp_p) - sd_functions.f_vhp(
                                min_level, HDiff_FP, area_p, slp_p)
                        elif orderDitch == 2:
                            irrg_pond_max = (sd_functions.f_vhd(Hd2, HDiff_FD2, Wd2_up, Wd2_lw,
                                                                Wf1) - sd_functions.f_vhd(
                                min_level, HDiff_FD2, Wd2_up, Wd2_lw, Wf1)) * n1 * n2
                        else:
                            irrg_pond_max = (sd_functions.f_vhd(Hd1, HDiff_FD1, Wd1_up, Wd1_lw,
                                                                Wf2) - sd_functions.f_vhd(
                                min_level, HDiff_FD1, Wd1_up, Wd1_lw, Wf2)) * n1
                        if irrg_pond_max > irrg_v:
                            irrg_out = 0
                            irrg_pond = irrg  # in mm
                    ...
                    irrg_pond_p = 0
                I_need_df[column].iloc[date_n] = irrg
                # Adjust irrigation quantity and frequency from freshwater resources, in 25% dry conditions
                if (irrg_out > 0) & (dayofYear != irrg_doy):
                    prcp_cum = prcp_d1[column].iloc[date_n - 60: date_n].sum()
                    I_df1 = I_out_df[column].copy(deep=True)
                    I_df1.iloc[ponding_date] = 0
                    Irrg_cum = I_df1.iloc[date_n - 60: date_n].sum()
                    if Irrg_cum > prcp_cum * Irrg_rate[0]:
                        if prcp_cum == 0:
                            irrg_out = 0
                            irrg = irrg_out + irrg_pond
                        else:
                            irrg_adj = max(0, (Irrg_rate[1] - Irrg_cum / prcp_cum) / (Irrg_rate[1] - Irrg_rate[0]))
                            irrg_out = irrg_adj * irrg_out
                            irrg = irrg_out + irrg_pond
                    elif Irrg_cum + irrg_out > prcp_cum * Irrg_rate[1]:
                        irrg_out = prcp_cum * Irrg_rate[1] - Irrg_cum
                        irrg = irrg_out + irrg_pond
                if havePond is True:
                    if orderDitch == 2:
                        TN_I_pond_df[column].iloc[date_n] = TN_cp * irrg_pond_p + TN_cd2 * irrg_pond_d  # in mg/m2
                ...
                TP_I_out_df[column].iloc[date_n] = TP_I * irrg_out  # in mg/m2
                I_df[column].iloc[date_n] = irrg  # in mm
                I_out_df[column].iloc[date_n] = irrg_out  # in mm
                I_pond_df[column].iloc[date_n] = irrg_pond  # in mm
                # Calculate lateral seepage to ditches
                if Hf > 0:
                    if orderDitch == 2:
    ...
    with open(output_dir + '/I_need_A{}.txt'.format(run_id), 'w') as f:
        f.write("The irrigated water content needed for rice growing is simulated as below, in mm.\n")
        f.close()
    irrg_need_A.to_csv(output_dir + '/I_need_A{}.txt'.format(run_id), mode='a', header=True, index=True, sep='\t',
                       float_format='%.2f')
    with open(output_dir + '/I_need_df.txt', 'w') as f:
        f.write("The irrigated water content needed for rice growing is simulated as below, in mm.\n")
        f.close()
